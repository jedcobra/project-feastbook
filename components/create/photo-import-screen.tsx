...
import { parseRecipeFromText, recognizeText } from '@/lib/photo-import';

type Stage = 'pick' | 'reading' | 'failed';

// Photographs a printed page or a handwritten card, reads it with
// client-side OCR (Tesseract.js — no server, no API key), and seeds a
// draft the same way the URL importer does. OCR on handwriting is
// hit-or-miss, which is exactly why this always lands in the composer for
...
export function PhotoImportScreen() {
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const draft = draftFromImport(parsed, 'photo');
      saveDraft(draft);
      ...
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center px-8 text-center">
        ...
        {stage === 'pick' && (
          <>
            ...
            <div className="mb-5 max-w-[270px] font-mono text-[12.5px] leading-[1.65] text-ink-mute">
              We read the photo right on your device, then you confirm every field before it saves — printed
              text comes through much more reliably than handwriting.
            </div>
            <button
              type="button"
        ...
        {stage === 'reading' && (
          <>
            ...
            <div className="mb-5 h-1.5 w-[200px] overflow-hidden rounded-full border border-ink">
              <div
                className="h-full bg-ink transition-[width]"
                style={{ width: `${Math.max(6, Math.round(progress * 100))}%` }}
              />
            </div>
            <div className="font-mono text-[11.5px] text-ink-mute">This can take a moment on a bigger photo.</div>
          </>
        )}
